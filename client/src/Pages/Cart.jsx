...
const Cart = ({ onCloseCart, onOpenCheckout }) => {
  ...
  const cartTotalRedux = useSelector((state) => state.cart.totalAmount) ?? 0;
  const shipping = 5.9;
  const discount = 0;
  const subTotal = Number(cartTotalRedux || 0).toFixed(2);
  const TotalPrice = (Number(cartTotalRedux || 0) + Number(shipping) - Number(discount)).toFixed(2);
              ...
              <Product key={product.id || product._id}>
                <ProductDetail>
                  ...
                  <Details>
                    ...
                    <ProductId>
                      <b>Unit Price:</b> €{Number(product.price || 0).toFixed(2)}
                    </ProductId>
                    <ProductId>
                      <b>Quantity:</b> {product.amount || 0}
                    </ProductId>
                    <ProductId>
                      <b>ID:</b> {product.id || product._id}
                    </ProductId>
                  </Details>
                </ProductDetail>
                <PriceDetail>
                  <ProductAmountContainer>
                    <Remove onClick={cartItemRemoveHandler.bind(null, product.id || product._id)} />
                    <ProductAmount>|</ProductAmount>
                    <Add onClick={cartItemAddHandler.bind(null, product)} />
                  </ProductAmountContainer>
                  <ProductPrice>€{(Number(product.price || 0) * Number(product.amount || 0)).toFixed(2)}</ProductPrice>
                </PriceDetail>
              </Product>
            ))}
            <Hr />
