      ...
      <ContainerTop>
        <Container>
          <ContainerTopInner>
            ...
            {/* top bar keeps social icons and auth links only; account avatar moved to main bar */}
            {/* Avatar replaces the old "My Account" text link */}
          </ContainerTopInner>
        </Container>
      </ContainerTop>
      <ContainerMid>
        <Container>
          <MainBarContainer>
              ...
              {isAdmin && (
                <>
                  <Link to="/service/add" style={{ textDecoration: "none" }}>
                    <PageLink>Add Service</PageLink>
                  </Link>
                  <Link to="/product/add" style={{ textDecoration: "none" }}>
                    <PageLink>Add Product</PageLink>
                  </Link>
                  <Link to="/worker/add" style={{ textDecoration: "none" }}>
                    <PageLink>Add Worker</PageLink>
                  </Link>
                  <Link to="/admin/workers" style={{ textDecoration: "none" }}>
                    <PageLink>Workers</PageLink>
                  </Link>
                </>
              )}
            </Nav>
            ...
            <AccountContainer>
              {isLogged ? (
                  <Link to={isAdmin ? "/admin" : "/user"} style={{ textDecoration: "none" }} aria-label="My account" title="My account">
                    {userImg ? (
                      <Avatar src={userImg} alt="user" />
                    ) : (
                      <InitialsAvatar name={name || localStorage.getItem('userName')} size={48} />
                    )}
                  </Link>
                ) : (
                  <Link to="/auth" style={{ textDecoration: "none" }}>
