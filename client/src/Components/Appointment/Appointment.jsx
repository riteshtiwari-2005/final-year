...
const Appointment = (props) => {
  ...
  const handleToggleTime = (value) => {
    // value is worker._id (string) or empty string
    setMechanic(value);
    ...
    // mark which slotTimes are already used for this worker on the selected date
    // slots is an array of objects with { mechanic, slotTime, slotDate }
    if (value) {
      slots.forEach((slot) => {
        if (formattedDate === slot.slotDate && slot.mechanic === value) {
